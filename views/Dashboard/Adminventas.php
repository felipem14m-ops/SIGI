...
$productos = []; $categorias = []; $metodos_pago = [];
...
try {
    ...
    $resumen_hoy = (new Venta($db))->resumenDiario();
    ...
    <div class="module-banner">
        <div class="d-flex align-items-center gap-3 position-relative" style="z-index: 2;">
            ...
            <div>
                ...
                <h2 class="fw-bold m-0 text-white" style="font-family: 'Poppins', sans-serif; letter-spacing: -1px; font-size: 1.8rem;">Punto de Venta (Admin)</h2>
                <p class="m-0 text-white opacity-90 fw-light small">Supervisión y ejecución de transacciones comerciales.</p>
            </div>
        </div>
        <div class="text-end position-relative" style="z-index: 2;">
            <div class="small fw-bold text-uppercase opacity-75" style="font-size: 0.65rem; letter-spacing: 1px; color: rgba(255,255,255,0.8);">Ingresos de Hoy</div>
            ...
            <tbody>
                <?php foreach($productos as $p): ?>
                    ...
                    <tr>
                        ...
                        <td class="fw-bold text-dark">$<?= number_format($p['precio_venta'], 2) ?></td>
...
$(document).ready(function() {
    ...
    $('#posCatFilter').on('change', function() {
        const val = $(this).val();
        // Filtramos en la columna 1 (donde está el nombre y categoría)
...
function agregarAlCarrito(prod) {
    ...
    if(existe) {
        if(existe.cantidad + 1 > prod.stock_actual) {
            Swal.fire({ icon: 'warning', title: 'Stock Insuficiente', timer: 1500, showConfirmButton: false });
            return;
        }
        existe.cantidad++;
    } else {
...
function renderCarrito() {
    ...
    $('#cartTotal').text('$' + total.toLocaleString('es-CO', {minimumFractionDigits: 2}));
}
        ...
        const { value: monto } = await Swal.fire({
            ...
            html: `
                <div style="text-align: left; margin-bottom: 1.5rem;">
                    <div style="background: #f8fafc; padding: 1rem; border-radius: 12px; margin-bottom: 1rem;">
                        ...
                        <div style="font-size: 2rem; font-weight: 800; color: #2563eb;">$${totalVenta.toLocaleString('es-CO', {minimumFractionDigits: 2})}</div>
                    </div>
                    ...
                    <input type="number" id="montoRecibidoInput" class="swal2-input" 
                           placeholder="0.00" step="0.01" min="${totalVenta}" 
            ...
            preConfirm: () => {
                ...
                if (montoVal < totalVenta) {
                    Swal.showValidationMessage(`El monto debe ser mayor o igual a $${totalVenta.toLocaleString('es-CO', {minimumFractionDigits: 2})}`);
                    return false;
                }
                return montoVal;
            }
            ...
            <div style="text-align: left;">
                <div style="background: #f8fafc; padding: 1.25rem; border-radius: 12px; margin-bottom: 1rem;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.75rem; padding-bottom: 0.75rem; border-bottom: 2px dashed #e2e8f0;">
                        ...
                        <span style="font-weight: 800; color: #0f172a; font-size: 1.1rem;">$${totalVenta.toLocaleString('es-CO', {minimumFractionDigits: 2})}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 0.75rem; padding-bottom: 0.75rem; border-bottom: 2px dashed #e2e8f0;">
                        <span style="font-weight: 600; color: #64748b;">Recibido:</span>
                        <span style="font-weight: 800; color: #0f172a; font-size: 1.1rem;">$${montoRecibido.toLocaleString('es-CO', {minimumFractionDigits: 2})}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-weight: 700; color: #2563eb; font-size: 1rem;">CAMBIO:</span>
                        <span style="font-weight: 900; color: #10b981; font-size: 1.8rem;">$${cambio.toLocaleString('es-CO', {minimumFractionDigits: 2})}</span>
                    </div>
                </div>
